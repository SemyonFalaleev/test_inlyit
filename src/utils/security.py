...
from src.db.base import get_async_db, AsyncSession
...
def create_access_token(data: dict, expires_delta: timedelta = None) -> str:
    try:
        if data.get("is_admin") == False:
            ...
            is_admin = 1
        payload = {"sub": str(data.get("id")),
                   "id": data.get("id"),
                   "is_admin": is_admin}
        
        expire = datetime.utcnow() + (expires_delta or timedelta(minutes=settings.token_expires))
        payload.update({"exp": expire})
        return jwt.encode(
            payload,
            settings.secret_key_jwt,
            algorithm=settings.algoritm_jwt
        )
